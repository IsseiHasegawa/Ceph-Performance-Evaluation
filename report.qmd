...
Since `read_random_buffer_count` and `read_random_disk_count` describe internal BlueFS/BlueStore read paths, I used them as backend read-path indicators rather than as a direct user-data cache hit/miss ratio. In particular, `read_random_buffer_count` indicates reads processed through the internal buffer path, while `read_random_disk_count` indicates reads that reached disk.
...
For a 1 GB simulated cache, libCacheSim predicted an LRU miss ratio of 62.97%. The BlueFS/BlueStore counters, however, showed that many backend random-read events still reached disk. These two measurements should not be interpreted as a direct cache-miss comparison because libCacheSim models an idealized data-block cache, while the Ceph counters reflect internal backend read paths.
